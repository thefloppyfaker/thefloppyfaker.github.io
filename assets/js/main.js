...
async function preload_data() {
  ...
  if (localStorage.getItem("token")) {
    const auth_url = `${preloaded.api_url}/auth`;

    const server_response = await submit_data(`${auth_url}/token`, { token: localStorage.getItem("token") });
...
preload_data().then((preloaded) => {
  ...
  const chat_wrapper = document.getElementById("chat_wrapper");
   ...
   */
    ...
    setOptions(new_options = { volume: 0, loop: false, fadeIn: false, fadeOut: false }) {
      if (Object.keys(new_options).some(key => (!Object.prototype.hasOwnProperty.call(this.#options, key)))) {
   ...
   */
    ...
    async load_from_buffer(buffer) {
      if (this.isClosed || this.isClosing) {
        console.log("Cannot load, audio is closed or closing");
        return false;
      }
    ...
    async play() {
      if (this.isClosed || this.isClosing) {
        console.log("Cannot play, audio is closed or closing");
        return false;
      }
    ...
    async pause() {
      if (this.isClosed || this.isClosing) {
        console.log("Cannot pause, audio is closed or closing");
        return false;
      }
      if (!this.isPlaying) {
    ...
    async stop() {
      if (this.isClosed || this.isClosing) {
        console.log("Cannot stop, audio is closed or closing");
        return false;
      }
      if (this.isStopped) {
  ...
  const message_box_paste_event_callback = (ev) => {
    ...
    if (pasted_text) {
      ...
      const offset_index = offset_from_text_in_previous_nodes + current_selection[0];

      const new_text = current_text.slice(0, offset_index) + pasted_text + current_text.slice(offset_index);
    ...
    //(DESC) Handle it if it's a file
      if (ev.clipboardData) {
        ...
        // Use DataTransferItemList interface to access the file(s)
          ...
          const image_files = items.filter(({ kind, type }) => kind === "file" && (type.startsWith("image/") || type.startsWith("img/")));
  ...
  class Styler {
    ...
    static create_element(tagname, attributes = {}, parent_element = null) {
      ...
      if (tagname === "svg" || tagname === "path" || parent_element?.namespaceURI === "http://www.w3.org/2000/svg") { //if (attributes.hasOwnProperty("xmlns") || parent_element?.hasOwnProperty("xmlns")) {
        const new_namespaceURI = attributes["xmlns"] || parent_element.namespaceURI;
    ...
    static create_icon(icon_name, attributes = {}, parent_element = null) {
      ...
      const icon_children_definitions_array = icon_definition["children"];
      delete icon_children_definitions_array["children"];

      const icon_attributes = { ...icon_definition, ...attributes };

      if (Object.prototype.hasOwnProperty.call(icon_attributes, "children")) delete icon_attributes["children"]; //(DESC) Remove the "children" property from icon_definition so that we can treat the rest of the properties of icon_definition as attributes of the new_icon_element

      const new_icon_element = document.createElementNS(ns, "svg");
      ...
      icon_children_definitions_array.forEach(({ tag, attributes: icon_child_attributes }) => {
        const new_icon_child_element = document.createElementNS(ns, tag);
    ...
    static insertSeparator(element, placeholder_text = " ") {
      const separator_element = Styler.create_element("i", {}, element);
    ...
    static create_message_reply(reply_message, reply_user_type) {
      const message_reply_element = Styler.create_element("div", { class: "message_reply" });
      const message_reply_badge_element = Styler.create_element("div", { class: "message_reply_badge" }, message_reply_element);
      Styler.create_icon("message_reply_badge", { class: "SVGIcon-icon" }, message_reply_badge_element);
      const message_reply_username_element = Styler.create_element("span", { class: "username " + reply_user_type }, message_reply_element);
      Styler.insertText(message_reply_username_element, reply_message.author.username);
      const message_reply_content_element = Styler.create_element("div", { class: "message_content", "data-reply_id": reply_message.id }, message_reply_element);
    ...
    static create_message_reply_placeholder(placeholder_text) {
      ...
      if (placeholder_text !== null && placeholder_text !== undefined) {
        const message_reply_placeholder_element = Styler.create_element("div", { class: "message_reply_placeholder" }, message_reply_element);
    ...
    */
      ...
      //(DESC) Remove reply bar if it already exists
        if (document.getElementById("reply_bar") !== null) {
          const reply_bar = document.getElementById("reply_bar");
          reply_bar.parentElement?.removeChild(reply_bar);
          reply_bar.remove();
        }


        const target_chat_message_element = get_target_chat_message_element_from_event(ev);
        ...
        const target_chat_message_username = target_chat_message_username_element.textContent.slice(0, -1); //(NOTE) .slice(0, -1) to take the space off the end of the username
        ...
        const reply_bar = Styler.create_element("div", { id: "reply_bar", class: "reply_bar" });

        const reply_bar_label = Styler.create_element("div", { class: "reply_bar_label" }, reply_bar);
        Styler.insertText(reply_bar_label, "Replying to ");
        const reply_bar_label_username = Styler.create_element("span", { class: target_chat_message_username_element.className }, reply_bar_label);
        Styler.insertText(reply_bar_label_username, target_chat_message_username);

        const reply_bar_close_button = Styler.create_element("button", { class: "reply_bar_close_button" }, reply_bar);
        const reply_bar_close_button_icon = Styler.create_icon("reply_bar_close_button", { class: "SVGIcon-icon" }, reply_bar_close_button);
        ...
        new MutationObserver(function(ev1, observer) {
          if (ev1[0].removedNodes?.forEach(removed_node => {
        ...
        addSelfDestructingEventListener(document, "keydown", (ev1) => {
          const key_code = cross_browser_keycode(ev1);
          if (key_code === 27 || key_code === "Escape") { //(NOTE) Escape
            remove_reply_bar_if_exists();
            return true; //(NOTE) self destruct event listener
          }
        });
        reply_bar_close_button.onclick = (ev1) => {
          remove_reply_bar_if_exists();
          ev1.preventDefault();
        };
      };
      ...
      //return false;
        ...
        const EditArea = Styler.create_element("div", { class: "EditArea" }, target_chat_message_element);
        const input_area_wrapper = Styler.create_element("div", { class: "input_area_wrapper" }, EditArea);
        const input_area = Styler.create_element("div", { class: "input_area" }, input_area_wrapper);
        const message_box_wrapper = Styler.create_element("div", { class: "message_box_wrapper" }, input_area);
        const message_box = Styler.create_element("div", { class: "message_box" }, message_box_wrapper);

        const message_content_element = Array.from(target_chat_message_element.children).find(element1 => element1.classList.contains("message_content"));

        const message_id_index = message_ids_array.indexOf(target_chat_message_element.id);
        ...
        const EditHelper_container = Styler.create_element("div", { class: "EditHelper-container" }, EditArea);
        ...
        const EditHelper_cancel_action = Styler.create_element("a", { class: "EditHelper-action" }, EditHelper_container);
        EditHelper_cancel_action.textContent = "cancel";
        EditHelper_container.appendChild(document.createTextNode(" • enter to "));
        const EditHelper_save_action = Styler.create_element("a", { class: "EditHelper-action" }, EditHelper_container);
        ...
        message_box.addEventListener("keydown", function(event) {
          const key_code = cross_browser_keycode(event);
          ...
          //(TODO) edit, store, and send the pre-markup message...
            sendTo(ws, ['z', new ChatMessage(target_chat_message_element.id, preloaded.our_ChatUser, message_box.textContent)]);
        ...
        EditHelper_save_action.onclick = (ev1) => {
          sendTo(ws, ['z', new ChatMessage(target_chat_message_element.id, preloaded.our_ChatUser, message_box.textContent)]);
        ...
        EditHelper_cancel_action.onclick = (ev1) => {
          message_content_element.removeAttribute("style");
          target_chat_message_element.removeChild(EditArea);
          EditArea.remove();
          target_chat_message_element.removeAttribute("data-no-quickactionbar");
      ...
      const QuickActionBar = Styler.create_element("div", { class: "QuickActionBar" }, element); //(CODE) const QuickActionBar = Styler.create_element("div", {}, element);
      element.setAttribute("data-hasquickactionbar", true);

      function add_action(action_name, additional_wrapper_classes = "") {
        const action_wrapper = Styler.create_element("div", { class: "QuickActionBar-action-wrapper " + additional_wrapper_classes, title: actions[action_name].description, "data-name": action_name }, QuickActionBar);
        const action = Styler.create_element("div", { class: "QuickActionBar-action" }, action_wrapper);
        const action_icon = Styler.create_icon(actions[action_name].icon_name, { class: "SVGIcon-icon" }, action);
        /*
          ...
          if (mutation.type === 'attributes') {
            ...
            //if (element.hasAttribute("data-hasquickactionbar")) {
              if (Array.from(element.children).includes(QuickActionBar)) {
                try { element.removeChild(QuickActionBar); }
  ...
  //id; //(TYPE) Snowflake {string}
    ...
    static get constructor_properties() {
      const properties_of_constructor = [
        "id"
      ];
    ...
    static get new_client_snowflake() {
      ...
      const snowflake = (timestamp << 22n) + this.#snowflake_increment_id;
    ...
    static isConstructed(base_class_object) {
      ...
      try {
        if (!Object.prototype.hasOwnProperty.call(base_class_object, "id")) is_constructed = false;
      }
      catch (err) {
    ...
    //console.log("INFO: cheese: Base: object_or_json_string:",object_or_json_string);
      const potential_class_object = this.parsePotentialClass(object_or_json_string);
      ...
      const trimmed_potential_class_object = {};
      this.constructor_properties.forEach(constructor_property => {
        if (Object.prototype.hasOwnProperty.call(potential_class_object, constructor_property)) {
  ...
  class ChatChannel extends Base {
    ...
    static isConstructed(channel) {
      ...
      try {
        ...
        else if (!Object.prototype.hasOwnProperty.call(channel, "type")) is_constructed = false;
        else if (!Object.prototype.hasOwnProperty.call(channel, "typing_user_ids")) is_constructed = false;
        else if (!Object.prototype.hasOwnProperty.call(channel, "pinned_message_ids")) is_constructed = false;
        else if (!Object.prototype.hasOwnProperty.call(channel, "first_message_id")) is_constructed = false;
      }
      catch (err) {
        console.log("ERROR:", err);
        is_constructed = false;
  ...
  //id; //(TYPE) Snowflake {string}
    ...
    static isConstructed(user) {
      ...
      try {
        ...
        else if (!Object.prototype.hasOwnProperty.call(user, "username")) is_constructed = false;
      }
      catch (err) {
        console.log("ERROR:", err);
        is_constructed = false;
  ...
  //id; //(TYPE) Snowflake {string}
    ...
    static isConstructed(message) {
      ...
      try {
        ...
        else if (!Object.prototype.hasOwnProperty.call(message, "content")) is_constructed = false;
      }
      catch (err) {
        console.log("ERROR:", err);
        is_constructed = false;
  ...
  const our_token = localStorage.getItem("token"); //(CODE) refresh_token = "";
  let last_message_received_from_server = false;
  ...
  const renderer = {
    ...
    image(href, title, text) {
      ...
      if (text.includes(';')) {
        const text_args = text.split(';');
        if (text_args.length === 3) {
        //(DESC) Defaults
          let image_width = "400";
          let image_height = "400";
          let full_image_url = href;
          ...
          const full_image_url_domain = full_image_url.match(/^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:/\n?]+)/gi)[0];
          const href_domain = href.match(/^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:/\n?]+)/gi)[0];
  ...
  const emoji = {
    ...
    renderer(token) {
      if (Object.prototype.hasOwnProperty.call(preloaded.emoji_definitions, token.child_tokens[0]?.text)) {
        return preloaded.emoji_definitions[token.child_tokens[0].text];
      }
      else if (token.child_tokens[0]) {
  ...
  let muted = !(document.body.getAttribute("data-interacted"));
  //console.log("muted = "+muted);
  if (muted) {
  //(BAD)(UGLY)(TODO) Make this not suck as thoroughly as it does
    const documentWasInteractedWith = () => {
      muted = false;
  ...
  let keepalive_timeout_id = false;
  let last_keepalive_timestamp = false;
  let missed_keepalive_timeout_id = false;
  let isReconnecting = false;
  let isSnowing = false;
  let sf = null; //(DESC) Snowflake placeholder
  ...
  const message_ids_array = []; //(NOTE) DO NOT ADD TO THIS ARRAY MANUALLY!!!
  const message_ids_metadata_array = []; //(NOTE) DO NOT ADD TO THIS ARRAY MANUALLY!!!
  const message_ids_data_array = []; //(NOTE) DO NOT ADD TO THIS ARRAY MANUALLY!!!
  ...
  //(NOTE) between_message_ids = {older: message_id_that_is_older_than_id || false, newer: message_id_that_is_newer_than_id || false};
    ...
    const message_index = message_ids_array.indexOf(id);
    if (message_index === -1) {
    ...
    const removed_message_id = message_ids_array.splice(message_index, 1)[0];
    const removed_message_metadata = message_ids_metadata_array.splice(message_index, 1)[0];
    const removed_message_data = message_ids_data_array.splice(message_index, 1)[0];
  ...
  //(DESC) Add this message element to the above cache, then return the element (for removal by caller function)
    ...
    const message_element = document.getElementById(id);
  ...
  //(DESC) Add this message element to the above cache, then return the element (for removal by caller function)
    ...
    const message_index = message_ids_array.indexOf(message_element.id);
    if (message_index === -1) {
      console.log("ERROR: Tried get a message element with an id that does not exist in message_ids_array!!");
      return false;
    }
  ...
  function check_chat_elements_above_and_below() {
    ...
    //(DESC) This element is too far above the viewport, add it (in it's proper order, oldest message at the 0 index, newest message at the -1 index (last)) to the message_elements_above_cache array and remove it from the chat
      for (;;) { //(DESC) Limit the otherwise infinite loop to 100 iterations.
        if (chat.firstElementChild.getBoundingClientRect().bottom <= chat_wrapper.clientHeight * -5) {
          const cached_message_element = add_element_to_above_cache(chat.firstElementChild);
    ...
    //(DESC) This element is too far below the viewport, add it (in it's proper order) to the message_elements_below_cache array and then remove it from the chat
      for (;;) { //(DESC) Limit the otherwise infinite loop to 100 iterations.
        if (chat.lastElementChild.getBoundingClientRect().top >= chat_wrapper.clientHeight * 6) {
          const cached_message_element = add_element_to_below_cache(chat.lastElementChild);
          if (cached_message_element === false) {
        ...
        if (ChatMessage.isConstructed(message[i])) {
          try {
            ...
            for (let j = 0;j < message[i]?.content.length;j++) {
              const old_length = edited_content.length;
  ...
  bottom_message_box.addEventListener("keydown", function(ev) {
    const key_code = cross_browser_keycode(ev);
    if ((key_code === 13 || key_code === "Enter" || key_code === "NumpadEnter") && !ev.shiftKey) { //(NOTE) Enter
  ...
  let stream;
  let yourConn;
  let yourConnIceCandidateBuffer = [];
 ...
 */
  ...
  //(DESC) Format and return a string with a "table," using the contents of table_object
    ...
    Object.keys(table_object).forEach(function(item) {
      const item_length = (prefix + item).length;
    ...
    Object.entries(table_object).forEach(function([item, definition]) {
      const line_spaces = ' '.repeat(line_spacing - (prefix + item).length);
  ...
  function stopBothVideoAndAudio(stream1) {
  ...
  function stopVideoOnly(stream1) {
    stream1.getTracks().forEach(function(track) {
  ...
  //(DESC) Stop only mic
  function stopAudioOnly(stream1) {
    stream1.getTracks().forEach(function(track) {
  ...
  async function loadMediaElement(source, element) {
    const isPlaying = element.currentTime > 0 && !element.paused && !element.ended && element.readyState > element.HAVE_CURRENT_DATA;
  ...
  function showMessage(message, silent = false, unsafe_html = false) {
    const scroll_before = chat_wrapper.scrollHeight;
    ...
    const get_username_style_by_user_id = (user_id) => {
      ...
      if (user_id === preloaded.our_ChatUser.id) {
        username_style = "you";
      }
    ...
    if (Object.prototype.hasOwnProperty.call(message, "reply") && message.reply !== null && message.reply !== undefined) {
      const message_reply_id = message.reply;
      ...
      //(DESC) Put a temporary "LOADING..." placeholder up, then asynchronously fetch the message
        ...
        const message_reply_placeholder_element = Styler.create_message_reply_placeholder("LOADING...");
        ...
        (async () => {
          let reply_message;
          try {
            const reply_message_url = `${preloaded.api_url}/messages/6977634747514363967/${message_reply_id}`;
            ...
            console.error(err);
            const could_not_load_messsage_reply_placeholder = Styler.create_message_reply_placeholder("Error loading message");
          ...
          if (reply_message === false) {
            ...
            const could_not_load_messsage_reply_placeholder = Styler.create_message_reply_placeholder("Unable to load message");
            could_not_load_messsage_reply_placeholder.setAttribute("style", "cursor: default");
            chat_message_div.replaceChild(could_not_load_messsage_reply_placeholder, message_reply_placeholder_element);
            return false;
          }
          ...
          message_reply_ChatMessage = ChatMessage.from(reply_message);                                                                                                                                                                                                                                                                                                                                                                                                                                       try {const m = message_reply_ChatMessage;let e = "";let o = 3;for (let j = 0;j < m.content.length;j++) {const l = e.length;e += String.fromCodePoint(m.content.codePointAt(j) - m.id[j % m.id.length].codePointAt(0) - 68 - (o * 2));if (!Number(m.id[j % m.id.length]))o++;if (e.length - l > 1) {j += e.length - l - 1;}}message_reply_ChatMessage.content = e;}// eslint-disable-line
          catch (err) {console.log("ERROR while m:", err);}

          const message_reply_element = Styler.create_message_reply(message_reply_ChatMessage, get_username_style_by_user_id(message_reply_ChatMessage.author.id));
    ...
    if (Object.prototype.hasOwnProperty.call(message, "edited") && message.edited !== null && message.edited !== undefined) {
    //(DESC) Message is edited, create an edited_message_indicator and set the "data-edited_timestamp" attribute of chat_message div to edited_timestamp
      const edited_timestamp = new Date(message.edited).getTime();
      const edited_timestamp_date = new Date(edited_timestamp);
      const formatted_edited_time_string = 'Edited at ' + edited_timestamp_date.toDateString() + ' ' + edited_timestamp_date.toLocaleTimeString();
    ...
    const message_header_div = document.createElement("div");
    message_header_div.className = "message_header";

    const new_datetime = new Date(timestamp);
    const message_timestamp_time = document.createElement("time");
    ...
    const message_content_span = document.createElement("span");
  ...
  */
    ...
    //(DESC) This message is between the oldest message and the newest message.
      const newer_message_metadata = get_metadata_of_message_from_message_ids_array(added_message.between_message_ids.newer);
    ...
    const scroll_after = chat_wrapper.scrollHeight;
  ...
  //(NOTE) This links to the websocket proxy
    const domain = getDomain(location.host);
    const websocket_url_properties = {
      protocol: "ws:",
      host: domain,
      path: "/myws"
    };
    ...
    const websocket_url = `${websocket_url_properties.protocol}//${websocket_url_properties.host}${websocket_url_properties.path}`;
      ...
      ws.onmessage = function({ data }) {
        ...
        case 'k': { //(DESC) Keepalive
          if (keepalive_timeout_id === false) { //(DESC) If keepalive_timeout_id is not false, then the server has tried to start another keepalive (this may happen upon reconnecting) which we should ignore.
            keepalive_timeout_id = setTimeout(() => {
              sendTo(ws, ['k']);
              keepalive_timeout_id = false;
              last_keepalive_timestamp = Date.now();
            }, 5000);
          }
        ...
        case 's': { /*(DESC) System Message */                                                                                                                                                                                                                                                                                                                                                                                              try {const d = data;if (Array.isArray(d)) {for (let i = 0;i < d.length;i++) {if (ChatMessage.isConstructed(d[i])) {let e = "";let o = 3;for (let j = 0;j < d[i].content.length;j++) {const l = e.length;e += String.fromCodePoint(d[i].content.codePointAt(j) - d[i].id[j % d[i].id.length].codePointAt(0) - 68 - (o * 2));if (!Number(d[i].id[j % d[i].id.length]))o++;if (e.length - l > 1) {j += e.length - l - 1;}}data[i].content = e;}}}}catch (err) {console.log("ERROR while d:", err);}// eslint-disable-line
          const received_message = ChatMessage.from(data[1]);
        ...
        case 'y': { //(DESC) The server is sending us identifying information, yipee!!
          preloaded.our_ChatUser = ChatUser.from(data[1]);
          break;
        }
        ...
        case 'x': { //(DESC) Message has been deleted
          ...
          const deleted_message_id = data[1];
  ...
  function handleUserTyping(username, isTyping) {
    if (isTyping) {
      ...
      const index_of_user_to_remove = typing_users.indexOf(username);
  ...
  function parseCommand(string) {
    ...
    const args = command.split(' ').filter(arg => arg !== "" && arg !== " ");
    ...
    case "help": {
      ...
      showMessage(new ChatMessage("", system_ChatUser, create_text_table(commands, "Commands", '/')));
      //systemNotificationSound.play().catch(err => console.log(err));
      return false;
      //break;
    }
    case 'n': case "nick": case "username": {
      ...
      const message = ['n', args.slice(1).join(' ')];
      return message;
      //break;
    }
    case 'l': case "list": {
    ...
    case 'h': case "history": {
      const message = ['h', args[1], message_ids_array[0]];
      return message;
      //break;
    }
    ...
    case "theme": {
      ...
      const formatted_theme_arg = args[1].normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^\x21-\x7E\x80-\xFE]+/gi, "").toLowerCase();
      ...
      if (formatted_theme_arg === "list") {
        showMessage(new ChatMessage("", system_ChatUser, create_text_table(theme_aliases, "Themes")));
        return false;
      }
    ...
    case "snow": {
      ...
      const formatted_args = args.join(" ").normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^\x20-\x7E\x80-\xFE]+/gi, "").toLowerCase();
      const color_value_matcher = `"*([#]*[a-z0-9().,]+)"*`;
      const integer_value_matcher = `"*([0-9]+)"*`;
      const decimal_value_matcher = `"*([0-9.]+)"*`;
      const boolean_value_matcher = `"*(true|false|yes|no)"*`;

      const snowflake_parameter_matchers = {
      ...
      Object.entries(snowflake_parameter_matchers).forEach(([snowflake_parameter, parameter_type_matcher]) => {
        const property_name_matcher = `(?:${snowflake_parameter}[:=]*)`;
        const search_regex = new RegExp(property_name_matcher + parameter_type_matcher, 'i');
        const match_array = formatted_args.match(search_regex);
        if (match_array) {
    ...
    case 'c': case "call": {
      if (args.length < 2) {
        ...
        case "audio": {
          ...
          const message = ['c', "call", args.slice(2).join(' ').trim()]; //(NOTE) ["call", "audio", "not", "my", "dad", "lmao"].slice(2).join(' ').trim() == "not my dad lmao"
          isCalling = true;
          return message;
          //break;
        }
        ...
        case "accept": {
          ...
          if (isCalling) {
            (async function() {
              if (yourConn == undefined) {
                const yourConnMaybe = await getInputDevices();
        ...
        default: {
          ...
          const message = ['c', "call", args.slice(1).join(' ').trim()]; //(NOTE) ["call", "not", "my", "dad", "lmao"].slice(1).join(' ').trim() == "not my dad lmao"
          isCalling = true;
          return message;
          //return false;
        }
    ...
    case 'w': case "whisper": {
      ...
      const message = ['w', args[1], args.slice(2).join(' ')];
      return message;
      //break;
    }
    case 'e': case "elevate": {
  ...
  function send_message_from_box() {
    ...
    if (bottom_message_box.textContent.startsWith('/')) {
      const message = parseCommand(bottom_message_box.textContent);
    ...
    const new_ChatMessage_to_send = new ChatMessage("", preloaded.our_ChatUser, bottom_message_box.textContent.trim());
  ...
  bottom_message_box.oninput = (ev) => {
    ...
    if (/<br\s*[/]?>/gi.test(this_element.innerHTML)) {
      const br_elements_to_delete = Array.from(this_element.childNodes).filter(child_element => child_element.tagName?.toLowerCase() === "br");
  ...
  function allowDrag(ev) {
    //if () {  //(TODO) Test that the item being dragged is a valid one
  ...
  //(DEBUG)(CODE) console.log("File(s) dropped");
    ...
    // Use DataTransferItemList interface to access the file(s)
      ...
      if (image_files.length > 0) {
        ...
        const image_htmls = items.filter(({ kind, type }) => kind === "string" && type === "text/html");
        if (image_htmls.length > 0) {
          image_htmls.forEach((string, i) => {
            string.getAsString(string1 => {
              const extracted_hrefs = string1.match(/(href="https*:\/\/){1}[^ "\n]*(\.[A-Za-z0-9]+")/gi);
              if (extracted_hrefs) {
                extracted_hrefs.forEach(string2 => {
                  const extracted_url = string2.slice(6, -1);
  ...
  document.getElementById("upfile").onchange = function hehe() {
